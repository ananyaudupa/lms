  ...
  return (
    <>
      <Box sx={{
        ...
        boxShadow: tokens.courseCardShadow, display: 'flex', alignItems: 'center', gap: 3,
      }}>
        <Button variant="contained" startIcon={<QuizIcon />} onClick={() => setQuizOpen(true)}
          sx={{ background: tokens.btnPrimary, textTransform: 'none', fontWeight: 700, borderRadius: 2, px: 2.5, py: 1.2, whiteSpace: 'nowrap', flexShrink: 0, '&:hover': { background: tokens.btnPrimaryHover } }}>
          Take A Quiz
        </Button>
          ...
          {levels.map((level, i) => (
            <Box key={level} sx={{ flex: 1, textAlign: 'center' }}>
              ...
              <LinearProgress variant="determinate" value={getLevelProgress(progress, i)} sx={{
                ...
                '& .MuiLinearProgress-bar': { background: `linear-gradient(90deg, ${tokens.progressBarFrom}, ${tokens.progressBarTo})`, borderRadius: 3 },
              }} />
            </Box>
          ))}
        </Box>
        ...
        <Button variant="contained" startIcon={<BarChartIcon />} onClick={() => setAnalyticsOpen(true)}
          sx={{ background: `linear-gradient(90deg, ${tokens.btnContinue}, #65dfff)`, textTransform: 'none', fontWeight: 700, color: '#10148f', borderRadius: 2, px: 2.5, py: 1.2, whiteSpace: 'nowrap', flexShrink: 0, '&:hover': { background: `linear-gradient(90deg, ${tokens.btnContinueHover}, #0ea5e9)` } }}>
          Progress
        </Button>
      </Box>
