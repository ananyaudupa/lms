...
type LevelData = { level: number; score: number | null; attempted: boolean };
type Props = {
  ...
  return (
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg" style={{ flexShrink: 0 }}>
      {bars.map((b, i) => {
        ...
        return (
          <g key={i}>
            <polygon points={front} fill="rgba(220,225,238,0.18)"/>
            <polygon points={right} fill="rgba(185,193,212,0.12)"/>
            <polygon points={top4}  fill="rgba(240,243,250,0.25)"/>
            <polygon points={front} fill="none" stroke="rgba(180,188,208,0.25)" strokeWidth="0.8"/>
            <polygon points={right} fill="none" stroke="rgba(180,188,208,0.25)" strokeWidth="0.8"/>
            <polygon points={top4}  fill="none" stroke="rgba(180,188,208,0.25)" strokeWidth="0.8"/>
          </g>
        );
      })}
    </svg>
    ...
    <Modal open={open} onClose={onClose}>
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          <Box>
            <Typography fontWeight={800} color={tokens.primary} sx={{ fontSize: { xs: '1.1rem', sm: '1.5rem' } }}>
        ...
        <Box sx={{
          borderRadius: 3, overflow: 'hidden', mb: 3, position: 'relative',
          background: 'linear-gradient(105deg,#060c2e 0%,#0e1a6e 30%,#5b0f6b 60%,#a8122a 85%,#c41230 100%)',
          p: { xs: 2, sm: 2.5 },
        }}>
          ...
          <Box sx={{ display: 'flex', gap: { xs: 2, sm: 4 }, mb: 2, position: 'relative', flexWrap: 'wrap' }}>
            {[{ val: `${attempted.length}/5`, label: 'Levels Attempted' }, { val: `${passed.length}`, label: 'Level Passed' }, { val: totalTimeTaken, label: 'Total Time Taken' }].map(s => (
              <Box key={s.label}>
                <Typography fontSize={{ xs: 20, sm: 24 }} fontWeight={800} color="#fff" lineHeight={1}>{s.val}</Typography>
        ...
        <Typography fontWeight={800} color={tokens.primary} sx={{ mb: 2, fontSize: { xs: '1rem', sm: '1.25rem' } }}>Level Details</Typography>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'flex-start' }}>
                ...
                <Box key={lvl.level} sx={{ display: 'flex', alignItems: 'stretch', gap: 2 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: 52 }}>
                    <Box sx={{
                      ...
                      background: isPassed ? `linear-gradient(135deg,#1565c0,${tokens.btnPrimary})` : isAttempted ? 'linear-gradient(135deg,#00b4d8,#48cae4)' : '#8b9099',
                      border: isLocked ? '2px solid #6b7280' : 'none',
                      boxShadow: isPassed ? '0 3px 12px rgba(21,101,192,0.4)' : isAttempted ? '0 3px 12px rgba(0,180,216,0.4)' : 'none',
                    }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 2 }, flex: 1, pb: isLast ? 0 : 1.8, flexWrap: 'wrap' }}>
                    <Box sx={{ flex: 1 }}>
                    ...
                    {!isLocked && (
                      <Box component="button"
                        ...
                        sx={{
                          background: `linear-gradient(90deg,#1e3a8a,${tokens.btnPrimary})`,
          ...
          {/* Hide decorative SVG on mobile */}
          <Box sx={{ alignSelf: 'flex-end', flexShrink: 0, display: { xs: 'none', sm: 'block' } }}>
            <Bars3DSVG />
          </Box>
        </Box>
      </Box>
