...
type Props = { attempt: AttemptLog };

export function AttemptRow({ attempt }: Props) {
  const [open, setOpen] = useState(attempt.attemptNumber === 1 && attempt.date !== null);
  const hasData = attempt.date !== null;

  return (
    <Box sx={{ border: '1px solid #e2e8f0', borderRadius: 3, mb: 1.5, overflow: 'hidden', background: '#fff' }}>

      {/* Row header */}
      <Box onClick={() => hasData && setOpen(v => !v)} sx={{
        display: 'flex', alignItems: 'center', gap: { xs: 1, sm: 2 },
        px: { xs: 2, sm: 3 }, py: { xs: 1.5, sm: 1.8 },
        ...
        {/* Time — hide on xs */}
        <Typography fontSize={14} color={hasData ? '#475569' : '#94a3b8'}
          sx={{ minWidth: 80, display: { xs: 'none', sm: 'block' } }}>
          {attempt.time ?? '---'}
        </Typography>
        ...
        {/* Incorrect */}
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, ml: { xs: 1, sm: 2 } }}>
        ...
        {/* Time taken — hide on xs */}
        <Box sx={{ display: { xs: 'none', sm: 'flex' }, alignItems: 'center', gap: 0.6, ml: 2 }}>
      ...
      <Collapse in={open}>
        {attempt.questions.length > 0 ? (
          <Box sx={{ px: { xs: 2, sm: 3 }, pt: 2, pb: 3, borderTop: '1px solid #f1f5f9', background: '#fafbfc' }}>
            {attempt.questions.map((q: import('../../data/explanation.data').QuestionResult) => (
              <QuestionBlock key={q.id} q={q} />
            ))}
          </Box>
        ) : (
          <Box sx={{ px: { xs: 2, sm: 3 }, py: 2, borderTop: '1px solid #f1f5f9' }}>
            <Typography fontSize={13} color="#94a3b8">No question data available for this attempt.</Typography>
          </Box>
        )}
      </Collapse>
    </Box>
